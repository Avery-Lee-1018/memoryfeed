...
class AnimationController {
  ...
  private readonly cycleDuration = 15;
  private readonly onCycleComplete?: () => void;

  constructor(ctx: CanvasRenderingContext2D, size: number, onCycleComplete?: () => void) {
    this.ctx = ctx;
    this.size = size;
  ...
  private setupTimeline() {
    this.timeline.to(this, {
      ...
      onComplete: () => this.onCycleComplete?.(),
    });
  }
...
class Star {
  ...
  constructor(cameraZ: number, cameraTravelDistance: number) {
    ...
    this.z = -200 + Math.random() * (cameraTravelDistance + cameraZ + 200);
    this.z = this.z * 0.7 + cameraTravelDistance * 0.15;
...
export function SpiralAnimation({ onCycleComplete }: SpiralAnimationProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const animationRef = useRef<AnimationController | null>(null);
  const onCycleCompleteRef = useRef<typeof onCycleComplete>(onCycleComplete);
  ...
  useEffect(() => {
    // Keep intro canvas dimensions stable during the single-cycle animation.
    // This prevents early timeline restarts caused by viewport resize jitter.
